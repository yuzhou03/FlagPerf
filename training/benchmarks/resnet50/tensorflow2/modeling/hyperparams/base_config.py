  ...
  """
    ...
    """
        if not isinstance(v, cls.SEQUENCE_TYPES):
            return False
        return (all(isinstance(e, cls.IMMUTABLE_TYPES) for e in v)
                or all(isinstance(e, dict) for e in v)
                or all(isinstance(e, params_dict.ParamsDict) for e in v))
        ...
        if k in cls.__annotations__:
            ...
            while traverse_in:
                ...
                if (isinstance(type_annotation, type)
                        and issubclass(type_annotation, Config)):
                    ...
                    if (isinstance(field_type, type)
                            and issubclass(field_type, cls.SEQUENCE_TYPES)):
                        element_type = typing.get_args(type_annotation)[0]
                        subconfig_type = (element_type if issubclass(
                            element_type, params_dict.ParamsDict) else
                                          subconfig_type)
                        break
    ...
    def __setattr__(self, k, v):
        ...
        if k not in self.RESERVED_ATTR:
            if getattr(self, '_locked', False):
                raise ValueError('The Config has been locked. '
            ...
            if k not in self.__dict__:
                ...
                if isinstance(v, dict) and self.__dict__[k]:
                    self.__dict__[k]._override(v, is_strict)  # pylint: disable=protected-access
                                ...
                                params_dict.ParamsDict) and self.__dict__[k]:
    ...
    """
        return {
            ...
            for k, v in self.__dict__.items() if k not in self.RESERVED_ATTR
        }
