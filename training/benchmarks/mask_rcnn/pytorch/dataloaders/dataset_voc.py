...
from utils.train import coco_remove_images_without_annotations, convert_coco_poly_mask
...
class CocoDetection(data.Dataset):
    ...
    def __init__(self, root, dataset="train", transforms=None, years="2017"):
        ...
        assert dataset in ["train",
                           "val"], 'dataset must be in ["train", "val"]'
        ...
        assert os.path.exists(
            self.img_root), "path '{}' does not exist.".format(self.img_root)
        self.anno_path = os.path.join(root, "annotations", anno_file)
        assert os.path.exists(
            self.anno_path), "file '{}' does not exist.".format(self.anno_path)
